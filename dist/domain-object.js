...
export class DomainObject extends Serializable {
    static _serializableKeysCache = new Map();
    /**
    ...
    constructor(data) {
        ...
        given(invalidKeys, "data")
            .ensure(t => t.length === 0, `data for class '${this.constructor.name}' contains properties [${invalidKeys.join(", ")}] that do not correspond to @serialize decorated getters`);
    }
    static _fetchSerializableKeys(ctor) {
        let keys = DomainObject._serializableKeysCache.get(ctor);
        if (keys == null) {
            const meta = ctor[symbolMetadata];
            const fields = (meta?.[serializableFieldsKey] ?? []);
